...
| `src/preload.js` | Context bridge: exposes (1.1.5: right-click menu; starts on staff.thinkopen.net) `window.minka.{isDesktop, app, version, platform, focusWindow, setBadge, onUpdateReady, installUpdate, getInventory, getPresence, openInBrowser}` (openInBrowser ≥ 1.1.4: https + our staff/support hosts only) |
...
- **macOS auto-update is ENABLED (since v1.0.10, `4e592d8` — build is signed + notarized).** It checks 8s after launch + every 6h, autoDownloads, fires `update-downloaded` → web "Update ready" modal → `quitAndInstall()`. NOT OS-silent like Windows: it waits on the user to click the modal, and **only works when the app lives in `/Applications`** (a translocated/quarantined launch can't self-replace). Auto checks swallow errors; the **"Check for Updates…" item (app menu + tray, v1.0.13)** is the manual trigger + visible status (Up to date / Downloading / Restart) and surfaces the otherwise-swallowed error. (Older note said "signing-gated, returns early on darwin" — that was true pre-1.0.10; the guard is gone.)
